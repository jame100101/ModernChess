...
public class ChessGame extends JFrame {
    ...
    private AmusementSelectionScreen amusementScreen;
    ...
    public ChessGame() {
        ...
        amusementScreen = new AmusementSelectionScreen(this);
        settingsScreen = new SettingsScreen(this);
        gameScreen = new GameScreen(this);
        
        mainPanel.add(startScreen, "START");
        mainPanel.add(difficultyScreen, "DIFFICULTY");
        mainPanel.add(amusementScreen, "AMUSEMENT");
    ...
    public void startGame(boolean vsAI, int difficulty) {
        // Default depth based on difficulty
        int depth = difficulty == 0 ? 2 : (difficulty <= 5 ? 5 : 12);
        startGameWithDepth(vsAI, difficulty, depth);
    }
    
    public void startGameWithDepth(boolean vsAI, int difficulty, int depth) {
        gameScreen.startNewGame(vsAI, difficulty, false, depth);
        cardLayout.show(mainPanel, "GAME");
        // Force layout refresh for game screen
    ...
    public void showAmusementSetup() {
        cardLayout.show(mainPanel, "AMUSEMENT");
    }
...
class StartScreen extends JPanel {
    ...
    public StartScreen(ChessGame frame) {
        ...
        amusementBtn.addActionListener(e -> mainFrame.showAmusementSetup());
    ...
    protected void paintComponent(Graphics g) {
        try {
            ...
            // 绘制棋子装饰 (Commented out to prevent crashes)
            ...
            e.printStackTrace();
            // Fallback: just fill with solid color
            g.setColor(new Color(26, 28, 36));
...
class DifficultySelectionScreen extends JPanel {
    ...
    public DifficultySelectionScreen(ChessGame frame) {
        ...
        // Data: Title, Description, SkillLevel, Depth
        Object[][] levels = {
            {"Beginner", "Skill 0, Depth 1<br>~800 Elo<br>Random Moves", 0, 1},
            {"Easy", "Skill 5, Depth 2<br>~1500 Elo<br>Amateur", 5, 5},
            {"Medium", "Skill 10, Depth 10<br>~2200 Elo<br>Master", 10, 10},
            {"Hard", "Skill 15, Depth 15<br>~2800 Elo<br>Grandmaster", 15, 15},
            {"God-like", "Skill 20, Depth 20+<br>3500+ Elo<br>Unbeatable", 20, 20}
        };
        ...
        for (int i = 0; i < levels.length; i++) {
            ...
            btn.addActionListener(e -> {
                mainFrame.startGameWithDepth(true, skill, depth);
            });
...
class AmusementSelectionScreen extends JPanel {
    private ChessGame mainFrame;
    private boolean isVsAI = false;
    private int selectedDifficulty = 5; // Default Medium

    // UI Components
    private JButton pvpBtn;
    private JButton pveBtn;
    private JPanel difficultyPanel;
    private JButton[] difficultyBtns;

    public AmusementSelectionScreen(ChessGame frame) {
        this.mainFrame = frame;
        setLayout(new BorderLayout());
        setBackground(new Color(26, 28, 36));

        // Header
        JLabel header = new JLabel("AMUSEMENT SETUP", SwingConstants.CENTER);
        header.setFont(new Font("Segoe UI", Font.BOLD, 48));
        header.setForeground(new Color(220, 220, 220));
        header.setBorder(BorderFactory.createEmptyBorder(60, 0, 40, 0));
        add(header, BorderLayout.NORTH);
        ...
        modeLabel.setFont(new Font("Segoe UI", Font.BOLD, 20));
        modeLabel.setForeground(new Color(180, 180, 180));
        modeLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        centerPanel.add(modeLabel);
        centerPanel.add(Box.createVerticalStrut(20));

        JPanel modePanel = new JPanel(new GridLayout(1, 2, 20, 0));
        modePanel.setOpaque(false);
        modePanel.setMaximumSize(new Dimension(600, 100));
        
        pvpBtn = createOptionButton("Player vs Player", true);
        pveBtn = createOptionButton("Player vs AI", false);
        
        pvpBtn.addActionListener(e -> selectMode(false));
        pveBtn.addActionListener(e -> selectMode(true));

        modePanel.add(pvpBtn);
        modePanel.add(pveBtn); // Corrected: was pvpBtn
        centerPanel.add(modePanel);
        centerPanel.add(Box.createVerticalStrut(40));

        // 2. Difficulty Selection (Hidden by default)
        difficultyPanel = new JPanel();
        difficultyPanel.setLayout(new BoxLayout(difficultyPanel, BoxLayout.Y_AXIS));
        difficultyPanel.setOpaque(false);
        
        JLabel diffLabel = new JLabel("AI Difficulty");
        diffLabel.setFont(new Font("Segoe UI", Font.BOLD, 20));
        ...
        JPanel diffBtnContainer = new JPanel(new GridLayout(1, 3, 15, 0));
        diffBtnContainer.setOpaque(false);
        diffBtnContainer.setMaximumSize(new Dimension(600, 80));

        difficultyBtns = new JButton[3];
        String[] diffNames = {"Easy", "Medium", "Hard"};
        int[] diffValues = {0, 5, 10};

        for(int i=0; i<3; i++) {
            final int difficulty = diffValues[i];
            final int index = i;
            difficultyBtns[i] = createOptionButton(diffNames[i], i==1); // Default Medium selected
            difficultyBtns[i].setFont(new Font("Segoe UI", Font.BOLD, 16));
            difficultyBtns[i].addActionListener(e -> selectDifficulty(index, difficulty));
            diffBtnContainer.add(difficultyBtns[i]);
        }
        
        difficultyPanel.add(diffBtnContainer);
        ...
        styleNavButton(backBtn, false);
        backBtn.addActionListener(e -> mainFrame.showMenu());

        JButton startBtn = new JButton("Start Game");
        styleNavButton(startBtn, true);
        startBtn.addActionListener(e -> mainFrame.startAmusementGame(isVsAI, selectedDifficulty));
        ...
        // Initialize state
        selectMode(false); // Default PvP
    }

    private JButton createOptionButton(String text, boolean active) {
    ...
    private void updateButtonStyle(JButton btn, boolean active) {
        if (active) {
            btn.setBackground(new Color(97, 218, 251));
            btn.setForeground(new Color(20, 20, 20));
            btn.setBorder(BorderFactory.createLineBorder(new Color(97, 218, 251), 2));
        } else {
            btn.setBackground(new Color(45, 48, 55));
            btn.setForeground(new Color(180, 180, 180));
            btn.setBorder(BorderFactory.createLineBorder(new Color(80, 83, 95), 1));
        }
    }

    private void styleNavButton(JButton btn, boolean primary) {
        ...
        if (primary) {
            ...
            // btn.setBorder(BorderFactory.createEmptyBorder()); // Removed to keep default look or set line border
    ...
    private void selectMode(boolean vsAI) {
        this.isVsAI = vsAI;
        updateButtonStyle(pvpBtn, !vsAI);
        updateButtonStyle(pveBtn, vsAI);
        difficultyPanel.setVisible(vsAI);
        revalidate();
        repaint();
    }

    private void selectDifficulty(int index, int value) {
        this.selectedDifficulty = value;
        for(int i=0; i<3; i++) {
            updateButtonStyle(difficultyBtns[i], i == index);
        }
    }
    ...
    protected void paintComponent(Graphics g) {
        try {
            ...
            Graphics2D g2 = (Graphics2D) g;
            g2.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
            GradientPaint gp = new GradientPaint(0, 0, new Color(26, 28, 36), 0, getHeight(), new Color(15, 16, 20));
            g2.setPaint(gp);
            g2.fillRect(0, 0, getWidth(), getHeight());
        } catch (Exception e) {
            // Fallback
...
class SettingsScreen extends JPanel {
    ...
    public SettingsScreen(ChessGame frame) {
        this.mainFrame = frame;
        setLayout(new BorderLayout());
        setBackground(new Color(26, 28, 36));
        ...
        soundLabel.setFont(new Font("Segoe UI", Font.BOLD, 24));
        soundLabel.setForeground(new Color(180, 180, 180));
        ...
        volumePanel.setMaximumSize(new Dimension(600, 150));

        int currentVolume = Math.round(SoundManager.getVolume() * 100f);
        JLabel volumeLabel = new JLabel("Volume: " + currentVolume + "%");
        volumeLabel.setFont(new Font("Segoe UI", Font.PLAIN, 18));
        volumeLabel.setForeground(new Color(200, 200, 200));
        volumeLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        volumePanel.add(volumeLabel);
        volumePanel.add(Box.createVerticalStrut(15));

        JSlider volumeSlider = new JSlider(0, 100, currentVolume);
        volumeSlider.setOpaque(false);
        volumeSlider.setForeground(new Color(97, 218, 251));
        volumeSlider.setMajorTickSpacing(25);
        volumeSlider.setMinorTickSpacing(5);
        volumeSlider.setPaintTicks(true);
        volumeSlider.setPaintLabels(true);
        volumeSlider.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        ...
        // Resolution Section
        JLabel resolutionLabel = new JLabel("Window Resolution");
        resolutionLabel.setFont(new Font("Segoe UI", Font.BOLD, 24));
        resolutionLabel.setForeground(new Color(180, 180, 180));
        resolutionLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        centerPanel.add(resolutionLabel);
        centerPanel.add(Box.createVerticalStrut(20));
        
        // Resolution buttons
        JPanel resolutionPanel = new JPanel(new GridLayout(2, 3, 15, 15));
        resolutionPanel.setOpaque(false);
        resolutionPanel.setMaximumSize(new Dimension(900, 150));
        
        String[][] resolutions = {
            {"900 x 700", "900", "700"},
            {"1200 x 900", "1200", "900"},
            {"1400 x 1000", "1400", "1000"},
            {"1600 x 1200", "1600", "1200"},
            {"Fullscreen", "-1", "-1"}
        };
        
        for (String[] res : resolutions) {
            JButton resBtn = new JButton(res[0]);
            resBtn.setFont(new Font("Segoe UI", Font.BOLD, 16));
            resBtn.setForeground(new Color(200, 200, 200));
            resBtn.setBackground(new Color(45, 48, 55));
            resBtn.setFocusPainted(false);
            resBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
            resBtn.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(80, 83, 95), 1),
                BorderFactory.createEmptyBorder(10, 10, 10, 10)
            ));
            
            final int width = Integer.parseInt(res[1]);
            final int height = Integer.parseInt(res[2]);
            
            resBtn.addMouseListener(new MouseAdapter() {
                public void mouseEntered(MouseEvent e) {
                    resBtn.setBackground(new Color(55, 58, 65));
                }
                public void mouseExited(MouseEvent e) {
                    resBtn.setBackground(new Color(45, 48, 55));
                }
            ...
            resolutionPanel.add(resBtn);
        }
        
        centerPanel.add(resolutionPanel);
        add(centerPanel, BorderLayout.CENTER);
...
class GameScreen extends JPanel {
    ...
    private JLabel whiteAvatarLabel;
    private JLabel blackAvatarLabel;
    private JLabel whiteNameLabel;
    private JLabel blackNameLabel;
    private java.awt.Image playerAvatar;
    private java.awt.Image aiEasyAvatar;
    private java.awt.Image aiMediumAvatar;
    private java.awt.Image aiHardAvatar;
    ...
    private void loadAvatars() {
        try {
            playerAvatar = javax.imageio.ImageIO.read(new java.io.File("assets/player.png"));
            aiEasyAvatar = javax.imageio.ImageIO.read(new java.io.File("assets/ai_easy.png"));
            aiMediumAvatar = javax.imageio.ImageIO.read(new java.io.File("assets/ai_medium.png"));
            aiHardAvatar = javax.imageio.ImageIO.read(new java.io.File("assets/ai_hard.png"));
        } catch (Exception e) {
            System.err.println("Failed to load avatar images: " + e.getMessage());
        }
    }
    ...
    private void createAvatarPanel() {
        ...
        avatarPanel.setBorder(BorderFactory.createEmptyBorder(10, 20, 10, 20));
        ...
        whiteAvatarLabel = new JLabel();
        whiteAvatarLabel.setPreferredSize(new Dimension(60, 60));
        whiteNameLabel = new JLabel("White");
        whiteNameLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
        whiteNameLabel.setForeground(Color.WHITE);
        
        whitePanel.add(whiteAvatarLabel);
        whitePanel.add(Box.createHorizontalStrut(10));
        whitePanel.add(whiteNameLabel);
        ...
        blackNameLabel = new JLabel("Black");
        blackNameLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
        blackNameLabel.setForeground(Color.WHITE);
        blackAvatarLabel = new JLabel();
        blackAvatarLabel.setPreferredSize(new Dimension(60, 60));
        
        blackPanel.add(blackNameLabel);
        blackPanel.add(Box.createHorizontalStrut(10));
        blackPanel.add(blackAvatarLabel);
        
        avatarPanel.add(whitePanel, BorderLayout.WEST);
        avatarPanel.add(blackPanel, BorderLayout.EAST);
    }
    
    private void updateAvatars(boolean vsAI, int difficulty) {
        // White is always the player (bottom)
        if (playerAvatar != null) {
            whiteAvatarLabel.setIcon(new ImageIcon(makeCircular(playerAvatar, 60)));
        }
        whiteNameLabel.setText("You (White)");
        
        // Black depends on game mode
        if (!vsAI) {
            // PvP mode
            if (playerAvatar != null) {
                blackAvatarLabel.setIcon(new ImageIcon(makeCircular(playerAvatar, 60)));
            }
            blackNameLabel.setText("Player 2 (Black)");
        } else {
            // PvE mode - select AI avatar based on difficulty
            java.awt.Image aiAvatar = null;
            String aiName = "AI";
            
            // Map difficulty to avatars:
            // 0-5: Easy (robot)
            // 10-15: Medium (student)
            // 20+: Hard (master)
            if (difficulty <= 5) {
                aiAvatar = aiEasyAvatar;
                aiName = difficulty == 0 ? "AI - Beginner" : "AI - Easy";
            } else if (difficulty <= 15) {
                aiAvatar = aiMediumAvatar;
                aiName = difficulty == 10 ? "AI - Medium" : "AI - Hard";
            } else {
                aiAvatar = aiHardAvatar;
                aiName = "AI - God-like";
            }
            
            if (aiAvatar != null) {
                blackAvatarLabel.setIcon(new ImageIcon(makeCircular(aiAvatar, 60)));
            }
            blackNameLabel.setText(aiName + " (Black)");
        }
    }
    
    private java.awt.Image makeCircular(java.awt.Image source, int size) {
        java.awt.image.BufferedImage output = new java.awt.image.BufferedImage(size, size, java.awt.image.BufferedImage.TYPE_INT_ARGB);
    ...
    // Force refresh board layout (called after window resize)
    ...
    public void startNewGame(boolean vsAI, int difficulty, boolean amusementMode, int multiplier) {
        ...
        if (!isVsAI) {
             sidePanel.setAIStatus("Mode: PvP", Color.CYAN);
        } else {
             int boardSize = logic.getBoardSize();
             if (isAmusementMode || boardSize != 8 || engineAI == null) {
                 sidePanel.setAIStatus("CPU: Java AI", Color.ORANGE);
             } else {
                 sidePanel.setAIStatus("CPU: Stockfish", Color.GREEN);
             }
        }
        ...
        // Update avatars based on game mode and difficulty
        updateAvatars(isVsAI, aiSkillLevel);
    }
    ...
    public void onTurnEnd() {
        ...
        // Check for Checkmate
        if (logic.isCheckmate(logic.isWhiteTurn)) {
            board.repaint();
            // The winner is the opponent of the current turn (who just moved)
            board.handleWin(!logic.isWhiteTurn);
            return;
        }
    ...
    private void performAIMove() {
        ...
        // 1. Determine if we should use internal AI
        boolean useInternalAI = isAmusementMode || boardSize != 8 || engineAI == null;
        ...
        if (useInternalAI) {
            ...
            engineAI.requestBestMoveAsync(logic, aiSkillLevel, new ChessAIController.AIResultListener() {
                ...
                public void onFailure(Exception e) {
                    System.err.println("Stockfish failed, falling back to internal AI: " + e.getMessage());
                    // Update Status to Red
                    SwingUtilities.invokeLater(() -> {
                         sidePanel.setAIStatus("CPU: Fallback (Java)", new Color(255, 80, 80));
                         runInternalAI();
                    });
                }
            });
    ...
    private void runInternalAI() {
        // Run internal AI on a background thread to prevent UI freezing
        new Thread(() -> {
            int depth = ChessAIController.mapSkillLevelToDepth(aiSkillLevel);
            // Cap depth to 4 for internal AI to ensure performance
            depth = Math.min(depth, 4);
            
            // Use a copy of logic to prevent UI flickering and history bloat
            Move bestMove = amusementAI.findBestMove(logic.copy(), depth);
            SwingUtilities.invokeLater(() -> applyAIMove(bestMove));
        }).start();
    }
    ...
    private void applyAIMove(Move bestMove) {
        // Guard: If it's now White's turn (e.g. user pressed Undo), ignore this stale AI move
        if (logic.isWhiteTurn) return;
...
class SidePanel extends JPanel {
    ...
    private JLabel aiStatusLabel; // New Status Label
    ...
    public SidePanel(GameScreen screen) {
        ...
        add(aiStatusLabel); // Add to layout
    ...
    public void setAIStatus(String text, Color color) {
        aiStatusLabel.setText(text);
        aiStatusLabel.setForeground(color);
    }
...
class ChessBoard extends JPanel {
    ...
    // Force refresh layout and repaint (called after window resize)
    ...
    public void handleWin(boolean whiteWon) {
        ...
        if (gameScreen.isVsAI()) {
            boolean humanWon = whiteWon; // Human is always White
            SoundManager.play(humanWon ? SoundEffect.WIN : SoundEffect.LOSE);
            if (!humanWon) title = "Defeat";
        } else {
...
class GameLogic {
    ...
    public boolean isSimulation = false; // Flag to prevent history recording and UI updates
    ...
    public GameLogic copy() {
        ...
        clone.boardSize = this.boardSize;
        clone.isWhiteTurn = this.isWhiteTurn;
        clone.isSimulation = true; // Enable simulation mode for the clone
        
        // Deep copy board
        clone.board = new Piece[boardSize][boardSize];
        for (int r = 0; r < boardSize; r++) {
            for (int c = 0; c < boardSize; c++) {
                if (this.board[r][c] != null) {
                    clone.board[r][c] = new Piece(this.board[r][c].type, this.board[r][c].isWhite);
                }
            }
        }
        
        // We don't need to copy history for AI simulation as checks don't rely on it (except undoLastMove which AI avoids)
    ...
    public Piece movePiece(int sr, int sc, int tr, int tc) {
        ...
        // Record for undo only if NOT in simulation mode
        if (!isSimulation) {
    ...
    public boolean isCheckmate(boolean isWhite) {
        ...
        // Try every possible move for every piece of 'isWhite'
        // If any move results in !isInCheck(isWhite), then it's not checkmate.
        for (int r = 0; r < boardSize; r++) {
            for (int c = 0; c < boardSize; c++) {
                Piece p = board[r][c];
                if (p != null && p.isWhite == isWhite) {
                    ...
                    for (Point m : moves) {
                        // Simulate move
                        // movePiece changes 'isWhiteTurn', pushes history
                        movePiece(r, c, m.y, m.x);
                        
                        // Check if still in check
                        boolean stillCheck = isInCheck(isWhite);
    ...
    private void sendCommand(String cmd) throws IOException {
        synchronized (ioLock) {
            if (!running || process == null || !process.isAlive()) {
                throw new IOException("Engine process is not running");
            }
            try {
                ...
                // If write fails, assume engine died
                running = false;
                throw e;
            }
        }
